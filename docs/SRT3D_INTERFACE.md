...
> **정리하면 인과는 이 방향이다.** "SRT3D라서 depth를 안 쓴다"가 아니라
> **"추적 단계에서 depth를 못 쓰는 구조라 region-only인 SRT3D가 맞는 선택이었다."**
> Galaxy XR은 depth 스트림을 추적 중에도 유지할 수 있으므로 M3T의 융합 모달리티가 의미를 갖는다.
